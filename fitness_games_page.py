...
from paths_to_images import PATH_TO_BACKGROUND_IMG, PATH_TO_KICK_AND_CATCH_BG_IMG, PATH_TO_YOGA_IMITATION_BG_IMG, PATH_TO_LEADERBOARD_ICON
...
def fitness_games_page(uname, window):
    ...
    from game_leaderboard import leaderboard_page
    ...
    leaderboard_img = Image.open(PATH_TO_LEADERBOARD_ICON)
    leaderboard_img = leaderboard_img.resize((ICON_WIDTH, ICON_HEIGHT))
    leaderboard_img = ImageTk.PhotoImage(leaderboard_img)
    leaderboard_button = tk.Button(window, font=button_font, text="Leaderboard", bg=button_bg, fg=button_fg, compound=RIGHT,
                                      activebackground=button_active_bg, image=leaderboard_img,
                                      command=lambda: leaderboard_page(uname, window))
